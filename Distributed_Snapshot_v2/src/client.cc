...
void Node::handleMessage(cMessage *msg)
{
        ...
        // leaf node and got msg of request data so now send reply msg to its parent
        ...
        // it executes every time irrespective of msg type
        // check  data received or not from all child
        if(isleaf==false)
        {
                bool alldone=true;
...
//                        MyMessage *retransmit=new MyMessage(retransmit);
//                        scheduleAt(simTime+2,retransmit);
                    }
                }
